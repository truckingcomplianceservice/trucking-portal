...
Truck photos: tap one to view full-size and flip through them all.
...
## New: truck photo gallery
On a vehicle's page, tap any photo to open it FULL-SIZE in a viewer. Then:
- Tap the ‹ and › arrows (or the buttons on screen) to go BACK / FORWARD through
  every photo of that truck, one at a time.
- On a phone you can also SWIPE left/right.
- On a computer you can use the LEFT/RIGHT arrow keys; ESC closes it.
- A counter shows "2 / 5" so you know where you are, and each photo's caption
  shows underneath.
- Tap × (or ESC) to close.

Add photos the same way as before (Photo + optional caption -> Add photo). The
header now shows how many photos the truck has.

## Includes everything to date
Truck photo gallery, office PWA + mobile layout, phone tap-to-call/text + phone
login + SMS-ready, driver nav + stop status + scanner, dismissible location
