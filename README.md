...
Fix: "hide load amounts" now hides on the settlement page too (not just the check).
...
## What was wrong
When you set "Hide load $ amounts" on a driver, it hid the amounts on the printed
CHECK but the on-screen SETTLEMENT page still showed the load rate and loads total
in a few spots (the loads table and the wage-calculator buttons). Confusing and
inconsistent.

## The fix
Now the driver-level "hide load amounts" setting hides load dollar amounts
EVERYWHERE for that driver: the settlement page's loads table, the loads total,
and the calculate-wages buttons -- as well as the printed check/stub. The loads
are still LISTED (routes/refs), and the NET PAY still shows (that's the actual pay).
A normal driver (setting off) still shows all amounts as before.

The per-settlement "Hide $ amounts" toggle button still works too; the driver
setting and the settlement toggle now both feed one consistent hide.

## Includes everything to date
Hide-load-amounts consistency fix, optional hide-load-amounts on check, check
amount alignment fix, owner-operator company pay + EIN on 1099, driver "pay checks
to" business name, IFTA CSV import, P&L partner breakdown, partner statement,
expense "Paid by" dropdown, partner ledger, loads on check stub, LMP100 check
layout, load photos gallery, driver check printing, invoice load search, invoice
auto-fill from load, P&L miles + $/mi, auto loaded-miles, invoice
