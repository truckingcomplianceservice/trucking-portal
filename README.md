...
Fix: itemized reimbursements/deductions now appear on the downloaded/printed PDF.
...
## What was wrong
The itemized reimbursement/deduction lines (with descriptions) showed on the
settlement screen, but the settlement PDF (Download PDF / Email to driver) was
built before that feature, so it left them out. Fixed.

## Now on the settlement PDF
The pay breakdown on the PDF now lists each itemized line with its description:
  + Lumper fee Miami        + $75.00
  - Trailer rent            - $200.00
...and the NET PAY reflects them. This applies to both "Download PDF" and the PDF
that gets emailed to the driver.

For browser PRINTING: the on-screen settlement already shows the items, so
printing the page (or using Download PDF then printing) includes them.
